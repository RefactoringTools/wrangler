...
%%io:format("Param: ~p ~n ", [lists:member({FunName, NewArity}, InscopeFuns)]),
  case (lists:member({FunName, NewArity}, InscopeFuns) or 
        ...
        and (Number > 1) of
    false -> ok; 
    true -> exit({error, atom_to_list(FunName) ++ "/" ++ integer_to_list(NewArity) 
                    ++ " is already in scope, "
...
check_def_mod(DefMod, ModName)->
  if DefMod == ModName -> ok;
     true -> exit({error, "This function is not defined in this module;" 
                          "please go to the module "
                          "where it is defined for tuple function parameters."})
  end.
  ...
  case lists:keysearch(module, 1, Info) of
    {value, {module, ClientModName}} ->
       InscopeFuns = lists:map(fun ({_M, F, A}) -> 
                                  {F, A} 
                                end, refac_util:inscope_funs(Info)),
       NewArity = Arity - N + 1,
       case (lists:member({Name, NewArity}, InscopeFuns) or 
             lists:member({Name, NewArity}, refac_util:auto_imported_bifs())) 
             and (Arity >  NewArity) of
         false ->  refac_util:stop_tdTP(fun do_tuple_fun_parameters/2, 
                                       Tree, {C, N, Name, Arity, Mod}); 
         true -> erlang:exit("The new function arity causes confliction in "
                             "client module: " ++ atom_to_list(ClientModName))
       end;
    _ -> refac_util:stop_tdTP(fun do_tuple_fun_parameters/2, 
                              Tree, {C, N, Name, Arity, Mod})
  end.
...
check_parameters(FirstPar, Number, Arity, FunPatterns, AppNode, AppPar) ->
  case ((Number < 1) or (Number > Arity)) of
    true -> 
      exit({error, "It is not possible to tuple " 
...
search_par(List, Number, First)->
  ...
  case (length(ParList) < Number) of
    true -> exit({error, "It is not possible to tuple  " 
                         ++ integer_to_list(Number) ++ " parameter"});
    false -> {lists:reverse(lists:nthtail(length(ParList)-Number, 
                                          lists:reverse(ParList))), C}
  end.
