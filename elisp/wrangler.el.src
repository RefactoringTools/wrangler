...
(defun my-ediff-qh()
  ...
  (if (equal modified-files nil)
      (commit-or-abort)
    (if (yes-or-no-p "Do you want to preview changes made to other files?")
	(progn
	  (setq file-to-diff (car modified-files))
	  (setq modified-files (cdr modified-files))
	  (ediff file-to-diff (concat file-to-diff ".swp")))
      (progn
	(setq modified-files nil)
	(commit-or-abort)))))
	...
	(erl-spawn
	  ...
	  ((['rex ['badrpc rsn]]
	   ...
	   (if (yes-or-no-p "Do you want to continue the refactoring?")
		 ...
		 (erl-receive (current-file-name line-no coulmn-no)
		     ((['rex ['badrpc rsn]]
		       (message "Refactoring failed: %S" rsn))
		      (['rex ['error rsn]]
		       (message "Refactoring failed: %s" rsn))
