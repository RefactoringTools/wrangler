...
\\[erl-refactor-rename-mod]       - Rename a module name.
;; \\[erl-refactor-tuple-to-record]  - From tuple to record.
  ...
  (interactive (list (erl-target-node)))
   (setq erlang-menu-items
       (erlang-menu-add-below 'refactor-menu-items
                              ...
                              erlang-menu-items))
   (erlang-menu-init)
   (erl-spawn (erl-send-rpc node 'distel 'start_undo_process (list)))
...
(defun erlang-refactor-off(node)
  ...
  (when (and erlang-refactor-mode node)
   (erl-spawn (erl-send-rpc node 'distel 'stop_undo_process (list)))
