-module(ets_eqc).

-include_lib("eqc/include/eqc.hrl").

-behaviour(general).

-export([table_name/0, module_name/0, particular_next_state_data/5,
	 delete_postcondition/0, insert_postcondition/0, set_up/0,
	 parallel_set_up/0, init_state/1, opened/1, get_contents/1,
	 particular_refactoring/1]).

-ifndef(BUG_OPEN_FILE).
-define(BUG_OPEN_FILE,false).
-endif.

-record(state,{name,type,contents=[]}).

table_name() -> dets_table.

module_name() -> dets.

%% Definition of the states. Each state is represented by a function, 
%% listing the transitions from that state, together with generators 
%% for the calls to make each transition.
init_state(_S) ->
    [ {opened,{call,ets,new,[ets_table,[public,named_table,oneof([set,bag])]]}}
     ].

opened(_S) ->
    [ {init_state,{call,ets,delete,[ets_table]}},
      {opened,{call,ets,insert,[ets_table,oneof([general:object(),list(general:object())])]}},
      {opened,{call,ets,insert_new,[ets_table,oneof([general:object(),list(general:object())])]}},
      {opened,{call,ets,delete,[ets_table,nat()]}},
      {opened,{call,?MODULE,get_contents,[ets_table]}}
     ].

%% Next state transformation for state data.
%% S is the current state, From and To are state names
particular_next_state_data(_From,_To,S,V,{call,_,new,[_,[public,named_table,T]]}) ->
    S#state{name=V,type=T};
particular_next_state_data(_From,_To,S,_V,{call,_,insert,[_,Objs]}) ->
    S#state{contents=general:model_insert(S#state.type,S#state.contents,Objs)};
particular_next_state_data(_From,_To,S,_V,{call,_,insert_new,[_,Objs]}) ->
    case general:any_exist(Objs,S#state.contents) of
	true ->
	    S;
	false ->
	    S#state{contents=general:model_insert(S#state.type,S#state.contents,Objs)}
    end;
particular_next_state_data(_From,_To,S,_V,{call,_,delete,[_,K]}) ->
    S#state{contents=general:model_delete(S#state.contents,K)};
particular_next_state_data(_From,_To,S,_V,{call,_,delete,[_]}) ->
    S#state{contents=[], type=undefined};
particular_next_state_data(_From,_To,S,_V,{call,_,_,_}) ->
    S.

delete_postcondition() -> true.

insert_postcondition() -> true.

set_up() -> catch(ets:delete(ets_table)).

parallel_set_up() -> catch(ets:delete(ets_table)).

get_contents(Name) ->
    ets:foldl(fun(X, Y)-> [X|Y] end, [], Name).

particular_refactoring(From) -> From==init_state.
