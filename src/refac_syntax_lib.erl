...
vann_match_expr(Tree, Env, Ms, VI) ->
    ...
    Tree3=constant_propagate(Tree2),
    {Tree3, Bound, Free}.
...
start_env_process() ->
    Pid = spawn_link(fun() -> env_loop([]) end), 
    register(value_env, Pid).
...
env_loop(Env) ->
	...
	{From, get, Key} ->
		...
		{value, {Key, Value}} -> From ! {value_env, {value, Value}};
		false -> From ! {value_env, false}
	    end,
	    env_loop(Env);
	{add, {Key, Value}} ->
