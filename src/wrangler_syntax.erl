...
%% @private
%% @hidden
-module(wrangler_syntax).

-export([abstract/1, add_ann/2, add_postcomments/2,
	 add_precomments/2, application/2, application/3,
	 application_arguments/1, application_operator/1,
	 arity_qualifier/2, arity_qualifier_argument/1,
	 ...
	 concrete/1, cond_expr/1, cond_expr_clauses/1,
	 conjunction/1, conjunction_body/1, cons/2, copy_ann/2,
	 copy_attrs/2, copy_comments/2, copy_pos/2, data/1,
	 disjunction/1, disjunction_body/1, eof_marker/0,
	 error_marker/1, error_marker_info/1,
	 flatten_form_list/1, float/1, float_literal/1,
	 float_value/1, form_list/1, form_list_elements/1,
	 fun_expr/1, fun_expr_arity/1, fun_expr_clauses/1,
	 function/2, function_arity/1, function_clauses/1,
	 function_name/1, function_clause/2, function_clause_name/1,
         function_clause/1, generator/2, generator_body/1,
	 generator_pattern/1, get_ann/1, get_attrs/1, get_pos/1,
	 get_postcomments/1, get_precomments/1, has_comments/1,
	 if_expr/1, if_expr_clauses/1, implicit_fun/1,
	 implicit_fun/2, implicit_fun_name/1, infix_expr/3,
	 infix_expr_left/1, infix_expr_operator/1,
	 ...
	 list_suffix/1, list_tail/1, macro/1, macro/2,
	 macro_arguments/1, macro_name/1, make_tree/2,
	 match_expr/2, match_expr_body/1, match_expr_pattern/1,
	 ...
	 named_fun_expr_name/1,
         map_elements/1,
         map_field_elements/1,
         nil/0, normalize_list/1,
	 ...
	 receive_expr/3, receive_expr_action/1,
	 receive_expr_clauses/1, receive_expr_timeout/1,
	 record_access/2, record_access/3,
	 record_access_argument/1, record_access_field/1,
	 record_access_type/1, record_expr/2, record_expr/3,
	 ...
	 try_expr_handlers/1, tuple/1, tuple_elements/1,
...
type(Node) ->
    ...
    case Node of
      ...
      {'fun', _, {function, _, _, _}} -> implicit_fun; %% Added by HL
      {'if', _, _} -> if_expr;
...
set_precomments(Node, Cs) ->
    case Node of
      ...
      _ -> set_precomments(wrap(Node), Cs)
    end.
...
add_precomments(Cs, Node) ->
    case Node of
      ...
      _ -> add_precomments(Cs, wrap(Node))
    end.
...
set_postcomments(Node, Cs) ->
    case Node of
      ...
      _ -> set_postcomments(wrap(Node), Cs)
    end.
...
add_postcomments(Cs, Node) ->
    case Node of
      ...
      _ -> add_postcomments(Cs, wrap(Node))
    end.
...
add_ann(A, Node) ->
    case Node of
      ...
      #wrapper{attr = Attr} ->
	  Node#wrapper{attr =
			   Attr#attr{ann = [A | Attr#attr.ann]}};
      _ ->
	  %% Assume we have an `erl_parse' node and create a wrapper
	  %% structure to carry the annotation.
	  Node  %%add_ann(A, wrap(Node))
    end.
...
get_attrs(Node) ->
    #attr{pos = get_pos(Node), ann = get_ann(Node),
	  com = get_com(Node)}.

%% =====================================================================
%% @spec set_attrs(Node::syntaxTree(),
%%                 Attributs::syntaxTreeAttributes()) -> syntaxTree()
%%
...
set_attrs(Node, Attr) ->
    case Node of
      #tree{} -> Node#tree{attr = Attr};
      #wrapper{} -> Node#wrapper{attr = Attr};
      _ -> set_attrs(wrap(Node), Attr)
    end.
...
copy_attrs(S, T) -> set_attrs(T, get_attrs(S)).

%% =====================================================================
%% @spec comment(Strings) -> syntaxTree()
%% @equiv comment(none, Strings)

comment(Strings) -> comment(none, Strings).
...
comment_text(Node) -> (data(Node))#comment.text.
...
comment_padding(Node) -> (data(Node))#comment.pad.
...
form_list(Forms) -> tree(form_list, [update_ann({syntax_path, form_list_element},F)||F<-Forms]).
...
form_list_elements(Node) -> data(Node).
...
flatten_form_list_1([F | Fs], As) ->
    case type(F) of
      ...
      _ -> flatten_form_list_1(Fs, [F | As])
    end;
flatten_form_list_1([], As) -> As.
...
variable(Name) when is_atom(Name) ->
    tree(variable, Name);
variable(Name) -> tree(variable, list_to_atom(Name)).
...
variable_literal(Node) ->
    case unwrap(Node) of
      {var, _, Name} -> atom_to_list(Name);
      Node1 -> atom_to_list(data(Node1))
    end.
...
underscore() -> tree(underscore, []).

revert_underscore(Node) ->
    Pos = get_pos(Node), {var, Pos, '_'}.
...
integer(Value) -> tree(integer, Value).
...
is_integer(Node, Value) ->
    case unwrap(Node) of
      {integer, _, Value} -> true;
      #tree{type = integer, data = Value} -> true;
      _ -> false
    end.
...
integer_value(Node) ->
	...
	true ->
	     Res;
	_ -> 
            %% list_to_integer does not work with based integers; this is only a temp fix.
            try list_to_integer(Res) of  
                V -> V
            catch 
                _E1:_E2_ -> Res
	     end
...
float(Value) -> make_float(Value).

make_float(Value) -> tree(float, Value).

revert_float(Node) ->
    Pos = get_pos(Node), {float, Pos, float_value(Node)}.
...
char(Char) -> tree(char, Char).

revert_char(Node) ->
    Pos = get_pos(Node), {char, Pos, char_value(Node)}.
...
is_char(Node, Value) ->
    case unwrap(Node) of
      {char, _, Value} -> true;
      #tree{type = char, data = Value} -> true;
      _ -> false
    end.
...
is_string(Node, Value) ->
    case unwrap(Node) of
      {string, _, Value} -> true;
      #tree{type = string, data = Value} -> true;
      _ -> false
    end.
...
atom(Name) when is_atom(Name) -> tree(atom, Name);
atom(Name) -> tree(atom, list_to_atom(Name)).

revert_atom(Node) ->
    Pos = get_pos(Node), {atom, Pos, atom_value(Node)}.
...
is_atom(Node, Value) ->
    case unwrap(Node) of
      {atom, _, Value} -> true;
      #tree{type = atom, data = Value} -> true;
      _ -> false
    end.
...
atom_name(Node) -> atom_to_list(atom_value(Node)).
...
tuple_size(Node) -> length(tuple_elements(Node)).
...
list(Elements, Tail) when Elements /= [] ->
    tree(list, #list{prefix = [update_ann({syntax_path, list_prefix}, E)||E<-Elements], 
                     suffix = update_ann({syntax_path, list_suffix}, Tail)}).
...
revert_list(Node) ->
    ...
    lists:foldr(fun (X, A) -> {cons, Pos, X, A} end, S, P).
...
revert_nil(Node) -> Pos = get_pos(Node), {nil, Pos}.
...
cons(Head, Tail) ->
    case type(Tail) of
      ...
      nil -> copy_comments(Tail, list([Head]));
      _ -> list([Head], Tail)
    end.
...
list_head(Node) -> hd(list_prefix(Node)).
...
list_length(Node) -> list_length(Node, 0).
...
normalize_list(Node) ->
    case type(Node) of
	  ...
	  P = list_prefix(Node),
	  case list_suffix(Node) of
	    none -> copy_attrs(Node, normalize_list_1(P, nil()));
	    Tail ->
...
binary(List) -> tree(binary, [update_ann({syntax_path, binary_field}, L) ||L<-List]).

revert_binary(Node) ->
    Pos = get_pos(Node), {bin, Pos, binary_fields(Node)}.
...
binary_field(Body) -> binary_field(Body, []).
...
binary_field_body(Node) ->
    case unwrap(Node) of
	  ...
	  if Size == default -> Body;
	     true -> size_qualifier(Body, Size)
	  end;
      Node1 -> (data(Node1))#binary_field.body
    end.
...
binary_field_size(Node) ->
    case unwrap(Node) of
	  ...
	  if Size == default -> none;
	     true -> Size
	  end;
      Node1 ->
	  Body = (data(Node1))#binary_field.body,
	  case type(Body) of
	    size_qualifier -> size_qualifier_argument(Body);
	    _ -> none
	  end
    end.
...
size_qualifier(Body, Size) ->
    copy_pos(Body, tree(size_qualifier,
	 #size_qualifier{body = update_ann({syntax_path, size_qualifier_body},Body), 
                         size = update_ann({syntax_path, size_qualifier_size}, Size)})).
...
error_marker(Error) -> tree(error_marker, Error).
...
eof_marker() -> tree(eof_marker).

revert_eof_marker(Node) ->
    Pos = get_pos(Node), {eof, Pos}.

%% =====================================================================
%% @spec attribute(Name) -> syntaxTree()
%% @equiv attribute(Name, none)

attribute(Name) -> attribute(Name, none).
...
revert_attribute_1(module, [M, List], Pos, Node) ->
    ...
    case revert_module_name(M) of
      {ok, A} -> {attribute, Pos, module, {A, Vs}};
      error -> Node
    end;
    ...
    case type(A) of
	  ...
	  case is_list_skeleton(List) of
		...
		case is_proper_list(List) of
		  true ->
		      Fs = fold_function_names(list_elements(List)),
		      {attribute, Pos, import, {concrete(A), Fs}};
		  false -> Node
		end;
	    false -> Node
	  end;
...
attribute_name(Node) ->
    case unwrap(Node) of
      {attribute, Pos, Name, _} -> set_pos(atom(Name), Pos);
      Node1 -> (data(Node1))#attribute.name
    end.
...
arity_qualifier(Body, Arity) ->
    tree(arity_qualifier,
	 #arity_qualifier{body = update_ann({syntax_path, arity_qualifier_body},Body), 
                          arity = update_ann({syntax_path, arity_qualifier_arity}, Arity)}).
...
module_qualifier(Module, Body) ->
    tree(module_qualifier,
	 #module_qualifier{module = update_ann({syntax_path, module_qualifier_argument}, Module), 
                           body = update_ann({syntax_path, module_qualifier_body}, Body)}).
...
module_qualifier_argument(Node) ->
    case unwrap(Node) of
      {remote, _, Module, _} -> Module;
      Node1 -> (data(Node1))#module_qualifier.module
    end.
...
module_qualifier_body(Node) ->
    case unwrap(Node) of
      {remote, _, _, Body} -> Body;
      Node1 -> (data(Node1))#module_qualifier.body
    end.
...
qualified_name(Segments) ->
    tree(qualified_name, [update_ann({syntax_path, qualified_name_segment}, S)
                          ||S<-Segments]).
...
%% Added by HL;
%%=============================================
-record(function_clause, {name, clause}).
function_clause(Name, Clause) ->
    set_pos(tree(function_clause, 
                 #function_clause{name=update_ann({syntax_path, function_name},Name), 
                                  clause=update_ann({syntax_path, function_clause},Clause)}),
            get_pos(Clause)).

function_clause_name(Node) ->
    (data(Node))#function_clause.name.

function_clause(Node) ->
    (data(Node))#function_clause.clause.
...
clause(Guard, Body) -> clause([], Guard, Body).
...
clause_guard(Node) ->
    case unwrap(Node) of
	  ...
	  case Guard of
	    ...
	    [L | _] when is_list(L) ->
		disjunction(conjunction_list(Guard));
	    [_ | _] -> conjunction(Guard)
	  end;
      Node1 -> (data(Node1))#clause.guard
    end.
...
clause_body(Node) ->
    case unwrap(Node) of
      {clause, _, _, _, Body} -> Body;
      Node1 -> (data(Node1))#clause.body
    end.
...
disjunction_body(Node) -> data(Node).
...
conjunction_body(Node) -> data(Node).
...
catch_expr(Expr) -> tree(catch_expr, update_ann({syntax_path, catch_expr_body}, Expr)).
...
match_expr(Pattern, Body) ->
    tree(match_expr,
	 #match_expr{pattern = update_ann({syntax_path, match_expr_pattern}, Pattern), 
                     body = update_ann({syntax_path, match_expr_body},Body)}).
...
match_expr_pattern(Node) ->
    case unwrap(Node) of
      {match, _, Pattern, _} -> Pattern;
      Node1 -> (data(Node1))#match_expr.pattern
    end.
...
match_expr_body(Node) ->
    case unwrap(Node) of
      {match, _, _, Body} -> Body;
      Node1 -> (data(Node1))#match_expr.body
    end.
...
%%     tree(operator, update_ann({syntax_path, operator_name}, Name));
%% operator(Name) -> tree(operator, update_ann({syntax_path, operator_name},list_to_atom(Name))).


operator(Name) when is_atom(Name) ->
    tree(operator,Name);
operator(Name) -> tree(operator, list_to_atom(Name)).
...
infix_expr(Left, Operator, Right) ->
    tree(infix_expr,
	 #infix_expr{operator = update_ann({syntax_path, infix_expr_operator},Operator), 
                     left = update_ann({syntax_path, infix_expr_left}, Left),
		     right = update_ann({syntax_path, infix_expr_right},Right)}).
...
infix_expr_left(Node) ->
    case unwrap(Node) of
      {op, _, _, Left, _} -> Left;
      Node1 -> (data(Node1))#infix_expr.left
    end.
...
infix_expr_operator(Node) ->
    case unwrap(Node) of
      ...
      Node1 -> (data(Node1))#infix_expr.operator
    end.
...
infix_expr_right(Node) ->
    case unwrap(Node) of
      {op, _, _, _, Right} -> Right;
      Node1 -> (data(Node1))#infix_expr.right
    end.
...
prefix_expr(Operator, Argument) ->
    tree(prefix_expr,
	 #prefix_expr{operator = update_ann({syntax_path, prefix_expr_operator},Operator), 
                      argument = update_ann({syntax_path, prefix_expr_argument},Argument)}).
...
prefix_expr_operator(Node) ->
    case unwrap(Node) of
      {op, Pos, Operator, _} ->
	  set_pos(operator(Operator), Pos);
      Node1 -> (data(Node1))#prefix_expr.operator
    end.
...
prefix_expr_argument(Node) ->
    case unwrap(Node) of
      {op, _, _, Argument} -> Argument;
      Node1 -> (data(Node1))#prefix_expr.argument
    end.
...
record_field(Name) -> record_field(Name, none).
...
record_field(Name, Value) ->
    tree(record_field,
	 #record_field{name = update_ann({syntax_path, record_field_name}, Name), 
                       value = update_ann({syntax_path, record_field_value}, Value)}).


-record(typed_record_field, {record_field, type}).


typed_record_field(RecordField, Type) ->
    tree(typed_record_field,
	 #typed_record_field{record_field=update_ann({syntax_path, record_field}, RecordField), 
                             type=update_ann({syntax_path, record_type},Type)}).

      
typed_record_field_name(Node) ->
    Field = (data(Node))#typed_record_field.record_field,
...
typed_record_field(Node) ->
    (data(Node))#typed_record_field.record_field.
...
record_index_expr(Type, Field) ->
    tree(record_index_expr,
	 #record_index_expr{type = update_ann({syntax_path, record_index_expr_type},Type), 
                            field = update_ann({syntax_path, record_index_expr_field},Field)}).
...
revert_record_index_expr(Node) ->
    ...
    case type(Type) of
      atom -> {record_index, Pos, concrete(Type), Field};
      _ -> Node
    end.
...
record_index_expr_type(Node) ->
    case unwrap(Node) of
      ...
      Node1 -> (data(Node1))#record_index_expr.type
    end.
...
record_index_expr_field(Node) ->
    case unwrap(Node) of
      {record_index, _, _, Field} -> Field;
      Node1 -> (data(Node1))#record_index_expr.field
    end.
...
record_access(Argument, Type, Field) ->
    tree(record_access,
	 #record_access{argument = update_ann({syntax_path, record_access_argument}, Argument),
                        type = update_ann({syntax_path, record_access_type}, Type),
                        field = update_ann({syntax_path, record_access_field}, Field)}).
...
revert_record_access(Node) ->
    ...
    if Type == none -> {record_field, Pos, Argument, Field};
       true ->
	   case type(Type) of
	     atom ->
...
record_access_argument(Node) ->
    case unwrap(Node) of
      {record_field, _, Argument, _} -> Argument;
      {record_field, _, Argument, _, _} -> Argument;
      Node1 -> (data(Node1))#record_access.argument
    end.
...
record_access_type(Node) ->
    case unwrap(Node) of
      {record_field, _, _, _} -> none;
      {record_field, Pos, _, Type, _} ->
	  set_pos(atom(Type), Pos);
      Node1 -> (data(Node1))#record_access.type
    end.
...
record_access_field(Node) ->
    case unwrap(Node) of
      {record_field, _, _, Field} -> Field;
      {record_field, _, _, _, Field} -> Field;
      Node1 -> (data(Node1))#record_access.field
    end.
...
record_expr(Argument, Type, Fields) ->
    tree(record_expr,
	 #record_expr{argument = update_ann({syntax_path, record_expr_argument},Argument),
                      type = update_ann({syntax_path, record_expr_type}, Type),
		      fields = [update_ann({syntax_path, record_expr_field}, F)|| F<-Fields]}).
...
record_expr_argument(Node) ->
    case unwrap(Node) of
      {record, _, _, _} -> none;
      {record, _, Argument, _, _} -> Argument;
      Node1 -> (data(Node1))#record_expr.argument
    end.
...
record_expr_type(Node) ->
    case unwrap(Node) of
      {record, Pos, Type, _} -> set_pos(atom(Type), Pos);
      {record, Pos, _, Type, _} -> set_pos(atom(Type), Pos);
      Node1 -> (data(Node1))#record_expr.type
    end.
...
record_expr_fields(Node) ->
    case unwrap(Node) of
      {record, _, _, Fields} -> unfold_record_fields(Fields);
      {record, _, _, _, Fields} ->
...
application(Operator, Arguments) ->
    tree(application,
	 #application{operator = update_ann({syntax_path, application_operator}, Operator),
		      arguments = [update_ann({syntax_path, application_argument}, A)||A<-Arguments]}).
...
application_operator(Node) ->
    case unwrap(Node) of
      {call, _, Operator, _} -> Operator;
      Node1 -> (data(Node1))#application.operator
    end.
...
application_arguments(Node) ->
    case unwrap(Node) of
      ...
      Node1 -> (data(Node1))#application.arguments
    end.
...
list_comp(Template, Body) ->
    tree(list_comp,
	 #list_comp{template = update_ann({syntax_path, list_comp_template},Template), 
                    body = [update_ann({syntax_path, list_comp_body}, B)||B<-Body]}).
...
list_comp_template(Node) ->
    case unwrap(Node) of
      {lc, _, Template, _} -> Template;
      Node1 -> (data(Node1))#list_comp.template
    end.
...
binary_comp(Template, Body) ->
    tree(binary_comp, #binary_comp{template = update_ann({syntax_path, binary_comp_template},Template), 
                                   body = [update_ann({syntax_path, binary_comp_body}, B) || B<-Body]}).
...
query_expr(Body) -> tree(query_expr, update_ann({syntax_path, query_expr_body}, Body)).
...
rule(Name, Clauses) ->
    tree(rule, #rule{name = update_ann({syntax_path, rule_name},Name), 
                     clauses =[update_ann({syntax_path, rule_clause}, C)||C<-Clauses]}).
...
rule_name(Node) ->
    case unwrap(Node) of
      {rule, Pos, Name, _, _} -> set_pos(atom(Name), Pos);
      Node1 -> (data(Node1))#rule.name
    end.
...
rule_clauses(Node) ->
    case unwrap(Node) of
      {rule, _, _, _, Clauses} -> Clauses;
      Node1 -> (data(Node1))#rule.clauses
    end.
...
generator(Pattern, Body) ->
    tree(generator,
	 #generator{pattern = update_ann({syntax_path, generator_pattern}, Pattern), 
                    body = update_ann({syntax_path, generator_body}, Body)}).
...
generator_pattern(Node) ->
    case unwrap(Node) of
      ...
      Node1 -> (data(Node1))#generator.pattern
    end.
...
binary_generator(Pattern, Body) ->
    tree(binary_generator, 
         #binary_generator{pattern = update_ann({syntax_path, binary_generator_pattern},Pattern), 
                           body = update_ann({syntax_path, binary_generator_body},Body)}).
...
block_expr(Body) -> tree(block_expr, [update_ann({syntax_path, body_expr}, B)||B<-Body]).
...
if_expr(Clauses) -> tree(if_expr, [update_ann({syntax_path, if_expr_clause}, C)||C<-Clauses]).
...
case_expr(Argument, Clauses) ->
    tree(case_expr,
	 #case_expr{argument = update_ann({syntax_path, case_expr_argument}, Argument), 
                    clauses = [update_ann({syntax_path, case_expr_clause}, C)||C<-Clauses]}).
...
case_expr_argument(Node) ->
    case unwrap(Node) of
      {'case', _, Argument, _} -> Argument;
      Node1 -> (data(Node1))#case_expr.argument
    end.
...
case_expr_clauses(Node) ->
    case unwrap(Node) of
      {'case', _, _, Clauses} -> Clauses;
      Node1 -> (data(Node1))#case_expr.clauses
    end.
...
cond_expr(Clauses) -> tree(cond_expr, [update_ann({syntax_path, cond_expr_clause}, C)||C<-Clauses]).
...
receive_expr(Clauses, Timeout, Action) ->
    ...
    tree(receive_expr,
	 #receive_expr{clauses = [update_ann({syntax_path, receive_expr_clause}, C)
                                  ||C<-Clauses], 
                       timeout = update_ann({syntax_path, receive_expr_timeout}, Timeout),
                       action = [update_ann({syntax_path, receive_expr_action}, A)||A<-Action1]}).
...
revert_receive_expr(Node) ->
    ...
    case Timeout of
      ...
      _ -> {'receive', Pos, Clauses, Timeout, Action}
    end.
...
receive_expr_clauses(Node) ->
    case unwrap(Node) of
      {'receive', _, Clauses} -> Clauses;
      {'receive', _, Clauses, _, _} -> Clauses;
      Node1 -> (data(Node1))#receive_expr.clauses
    end.
...
receive_expr_timeout(Node) ->
    case unwrap(Node) of
      {'receive', _, _} -> none;
      {'receive', _, _, Timeout, _} -> Timeout;
      Node1 -> (data(Node1))#receive_expr.timeout
    end.
...
receive_expr_action(Node) ->
    case unwrap(Node) of
      {'receive', _, _} -> [];
      {'receive', _, _, _, Action} -> Action;
      Node1 -> (data(Node1))#receive_expr.action
    end.
...
try_expr(Body, Clauses, Handlers, After) ->
    tree(try_expr,
	 #try_expr{body = [update_ann({syntax_path, body_expr}, B)||B<-Body], 
                   clauses = [update_ann({syntax_path, try_expr_clause}, C)||C<-Clauses],
		   handlers =[update_ann({syntax_path, try_expr_handler}, H)||H<-Handlers], 
                   'after' = [update_ann({syntax_path, body_expr}, B)||B<-After]}).
...
try_expr_body(Node) ->
    case unwrap(Node) of
      {'try', _, Body, _, _, _} -> Body;
      Node1 -> (data(Node1))#try_expr.body
    end.
...
try_expr_clauses(Node) ->
    case unwrap(Node) of
      {'try', _, _, Clauses, _, _} -> Clauses;
      Node1 -> (data(Node1))#try_expr.clauses
    end.
...
try_expr_handlers(Node) ->
    case unwrap(Node) of
      ...
      Node1 -> (data(Node1))#try_expr.handlers
    end.
...
try_expr_after(Node) ->
    case unwrap(Node) of
      {'try', _, _, _, _, After} -> After;
      Node1 -> (data(Node1))#try_expr.'after'
    end.
...
implicit_fun(Name, none) -> implicit_fun(Name);
...
implicit_fun(Name) -> tree(implicit_fun, update_ann({syntax_path, implicit_fun_name},Name)).
...
fun_expr(Clauses) -> tree(fun_expr, Clauses).
...
parentheses(Expr) -> tree(parentheses, update_ann({syntax_path, parentheses_body}, Expr)).

revert_parentheses(Node) -> parentheses_body(Node).


fake_parentheses(Expr) -> tree(fake_parentheses, update_ann({syntax_path, fake_parentheses_body}, Expr)).
...
empty_node() ->tree(empty_node, none).

%% =====================================================================
%% @spec macro(Name) -> syntaxTree()
%% @equiv macro(Name, none)

macro(Name) -> macro(Name, none).
...
macro_name(Node) -> (data(Node))#macro.name.
...
macro_arguments(Node) -> (data(Node))#macro.arguments.
...
abstract([H | T]) when is_integer(H) ->
    case is_printable([H | T]) of
      true -> string([H | T]);
      false -> abstract_tail(H, T)
    end;
abstract([H | T]) -> abstract_tail(H, T);
abstract(T) when is_atom(T) -> atom(T);
abstract(T) when is_integer(T) -> integer(T);
...
abstract_tail(H, T) -> cons(abstract(H), abstract(T)).
...
is_literal(T) ->
    case type(T) of
	  ...
	  case is_literal(list_head(T)) of
	    true -> is_literal(list_tail(T));
	    false -> false
	  end;
      tuple -> lists:all(fun is_literal/1, tuple_elements(T));
      _ -> false
    end.
...
subtrees(T) ->
    case is_leaf(T) of
      ...
      false ->
	    ...
	    cond_expr -> [cond_expr_clauses(T)];
	    conjunction -> [conjunction_body(T)];
	    disjunction -> [disjunction_body(T)];
	    form_list -> [form_list_elements(T)];
	    fun_expr -> [fun_expr_clauses(T)];
	    function -> [[function_name(T)], function_clauses(T)];
            function_clause ->[[function_clause_name(T)], [function_clause(T)]];
	    generator ->
		[[generator_pattern(T)], [generator_body(T)]];
	    if_expr -> [if_expr_clauses(T)];
	    implicit_fun -> [[implicit_fun_name(T)]];
	    infix_expr ->
	    ...
	    query_expr -> [[query_expr_body(T)]];
	    receive_expr ->
		case receive_expr_timeout(T) of
		  none -> [receive_expr_clauses(T)];
		  E ->
		...
		case record_access_type(T) of
		  none ->
		      [[record_access_argument(T)], [record_access_field(T)]];
		  R ->
		      [[record_access_argument(T)], [R],
		       [record_access_field(T)]]
		end;
	    record_expr ->
		case record_expr_argument(T) of
		  none -> [[record_expr_type(T)], record_expr_fields(T)];
		  V -> [[V], [record_expr_type(T)], record_expr_fields(T)]
		end;
	    record_field ->
	    ...
	    rule -> [[rule_name(T)], rule_clauses(T)];
	    size_qualifier ->
...
make_tree(function_clause, [[N], [C]]) -> function_clause(N, C);
...
meta_0(T) -> meta_1(remove_comments(T)).

meta_1(T) ->
    %% First handle leaf nodes and other common cases, in order to
    %% generate compact code.
    case type(T) of
      atom -> meta_call(atom, [T]);
      char -> meta_call(char, [T]);
      comment ->
	  meta_call(comment,
		    [list([string(S) || S <- comment_text(T)])]);
      eof_marker -> meta_call(eof_marker, []);
      error_marker ->
	  meta_call(error_marker,
		    [abstract(error_marker_info(T))]);
      float -> meta_call(float, [T]);
      integer -> meta_call(integer, [T]);
      nil -> meta_call(nil, []);
      operator ->
	  meta_call(operator, [atom(operator_name(T))]);
      string -> meta_call(string, [T]);
      text -> meta_call(text, [string(text_string(T))]);
      underscore -> meta_call(underscore, []);
      variable ->
...
meta_list([T | Ts]) -> [meta(T) | meta_list(Ts)];
meta_list([]) -> [].

meta_subtrees(Gs) ->
    list([list([meta(T) || T <- G]) || G <- Gs]).
...
tree(Type, Data) -> #tree{type = Type, data = Data}.
...
wrap(Node) ->
    ...
    #wrapper{type = type(Node),
	     attr = #attr{pos = get_pos(Node)}, tree = Node}.
...
is_printable(S) -> io_lib:printable_list(S).
...
fold_function_name(N) ->
    ...
    case (type(Name) == atom) and (type(Arity) == integer)
	of
      true -> {concrete(Name), concrete(Arity)}
    end.
...
fold_qualified_name([S | Ss], Pos) ->
    fold_qualified_name(Ss, Pos,
			{atom, Pos, atom_value(S)}).

fold_qualified_name([S | Ss], Pos, Ack) ->
    fold_qualified_name(Ss, Pos,
			{record_field, Pos, Ack, {atom, Pos, atom_value(S)}});
fold_qualified_name([], _Pos, Ack) -> Ack.
...
set_name(Node, Name) ->
    case Node of
      #wrapper{tree = Tree} ->
	  Node#wrapper{tree = set_name(Tree, Name)};
      #tree{data = Data} ->
	  Node#tree{data = set_name(Data, Name)};
      {var, Pos, _} -> {var, Pos, Name}
    end.


default_literals_vars(Node, Value) ->
    case Node of 
	#wrapper{tree = Tree} ->
	    Node#wrapper{tree = default_literals_vars(Tree, Value)};
	#tree{data = Data} ->
	    Node#tree{data = default_literals_vars(Data, Value)};
	{T, _Pos, _} -> {T, {0,0}, Value};
	{nil, _} -> {nil, {0,0}};
	_  -> Node      
    end.
